<?php
function comment_form_picking( $args = array(), $post_id = null ) {
	if ( null === $post_id ) {
		$post_id = get_the_ID();
	}
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment" class="row ml-0" style="font-size:0.9rem">' . _x( 'Comment', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="30" rows="3" maxlength="65525" required="required"></textarea></p>',
		...
		'submit_button'        => '<input name="%1$s" type="submit" id="%2$s" class="%3$s btn btn-dark btn-sm " value="%4$s" />',
